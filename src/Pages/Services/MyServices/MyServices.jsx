...
const getMyServiceData = async () => {
    ...
    // const url = `/myservice?email=${user?.email}`
        ...
        const res = await axiosSecure.get(`/myservice?email=${user?.email}`)
        .then(res=>res.data)
        return res;
  ...
  if (isError) {
    ...
    return (
        <div className="relative top-20 container lg:flex   mx-auto">
            My {services?.length} Service  availabe
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th scope="col" className="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Sort Description</th>
                        ...
                        {
               services.length === 0 ? <> loading.. </>: <> {services?.map(service=> <MyServiceTable key={service._id} service={service} />)}</>
            }
