...
const id = pendingData?._id
    ...
    return (
      ...
      { status=== 'Completed' ? <> <span className="font-bold text-primary">{status}</span> </>
      :
      <>{status === 'In Progress'? <><button onClick={()=>handleStatus(status,id)} className="p-4 bg-yellow-600 rounded-xl">{status}</button></>:<><button onClick={()=>handleStatus(status,id)} className="p-4 bg-red-500 rounded-xl">{status}</button></>}</> }
